...
There are multiple ways to install Jenkins on a K8s cluster. In this task you need to install Jenkins using a Helm chart. Note that Helm should be installed prior on your computer, where you have kubectl configured to access the cluster from the previous module. See HELM Documentation for more details on Helm installation. You may verify your Helm installation by deployment and removal of Nginx chart.

When done with Helm, install Jenkins using the instruction from Jenkins documentation. Make sure that Jenkins is installed into a separate namespace.
...
In this task you need to create HELM chart for Given Application. Using helm tool create a HELM chart for your application and adjust it to fit your needs.

A Docker image(s) for your application. The image(s) should be stored in the public image registry. Note, that your k8s nodes should be able to access this repository as well.
A Helm chart for your application. The chart should handle configuration of all the components (like configmaps, deployments, etc.), which are required to run the application withing a K8s cluster. See Helm documentation for more details on this step. During the development of the chart, you may test it manually from your computer, where Helm is installed.
This task is considered as done if all the conditions below are met:
...
Application's build;
Unit test's execution;
Security check with SonarQube;
Docker image building and pushing to public registry. This step is supposed be triggered manually;
Deployment of an application's build into your k8s cluster with Helm. This step should depend on the previous one.
(Optional) Application's verification (curl main page, send requests toward API if any, etc.) You may run a smoke test on this step as well.
This task is considered as done if all the conditions below are met:
